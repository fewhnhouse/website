  ...
  return (
    ...
    >
      ...
      {result?.status === 'needs_auth' ? (
        <div className="m-4 flex items-center justify-between gap-4 rounded-xl border border-[#fc4c02]/35 bg-[#fc4c02]/10 p-4 max-sm:flex-col max-sm:items-start">
          <div>
            <strong className="text-white">Connect Felix&apos;s Strava account</strong>
            <p className={`mt-1 mb-0 text-sm ${mutedTextClass}`}>{result.message}</p>
          </div>
          <a
            className="rounded-md bg-[#fc4c02] px-3 py-2 text-sm font-extrabold text-white no-underline hover:text-white"
